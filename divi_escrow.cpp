                     ...
                     const string         memo)
{
    ...
    // Enforce `sender` as sender & `approver` as DiviStock
    // Asserts should be removed once escrow.bos is ready for public use
    check(sender == name("sender"), "sender must be sender");
    check(approver == name("DiviStock"), "approver must be DiviStock");
    ...
    // Set Escrow deposit as `Divicoin` DIV (Extended Asset)
    extended_asset zero_asset{{0, symbol{"DIV", 4}}, "eosio.token"_n};


    // Sender can only have one un-filled escrow
    // Sender must either transfer DIV to `escrow.div` or `cancel` the existing escrow
    auto by_sender = escrows.get_index<"bysender"_n>();
    for (auto esc_itr = by_sender.lower_bound(sender.value), end_itr = by_sender.upper_bound(sender.value); esc_itr != end_itr; ++esc_itr) {
        check(esc_itr->ext_asset.quantity.amount != 0, "You already have an empty escrow.  Either transfer DIV to escrow.div or cancel the escrow");
    }
